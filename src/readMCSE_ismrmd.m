function [c_img, kdata, noise, info] = readMCSE_ismrmd(flags)
%   Read multiple SE dataset from ISMRMD .h5 and noise.h5
%   Input:
%       - flags: all info for reading

%   Output:
%   Saved as cell format, each cell contains one TE SE k-space or
%   images
...
%     info{nF}.ESP        = header.sequenceParameters.echo_spacing; % [msec]
    ...
    %% Extract non-trigger signal
    if flags.Trigger
%        inter_contrast_idx = find(diff(temp_raw_data.head.acquisition_time_stamp(1,:)) == (info{nF}.TE(1) / 2.5));   % ESP(ms) convert to time stamp
%        inter_slice_idx = inter_contrast_idx(diff(inter_contrast_idx) ~= 1)+1;
%        true_idx = sort([inter_contrast_idx,inter_slice_idx,max(inter_contrast_idx)+1]);
       true_idx = find(temp_raw_data.head.number_of_samples==header.encoding.encodedSpace.matrixSize.x);       
       info{nF}.rawData.number_of_samples  = single(max(temp_raw_data.head.number_of_samples(true_idx)));
       info{nF}.rawData.discard_pre        = single(max(temp_raw_data.head.discard_pre(true_idx)));
       info{nF}.rawData.discard_post       = single(max(temp_raw_data.head.discard_post(true_idx)));
       info{nF}.rawData.nr_samples         = info{nF}.rawData.number_of_samples - info{nF}.rawData.discard_pre  - info{nF}.rawData.discard_post;
       info{nF}.rawData.nr_channels        = single(max(temp_raw_data.head.active_channels(true_idx)));
       info{nF}.rawData.nr_phase_encodings = single(max(temp_raw_data.head.idx.kspace_encode_step_1(true_idx))) + 1; % nr_interleaves for spiral imaging
       info{nF}.rawData.nr_slice_encodings = single(max(temp_raw_data.head.idx.kspace_encode_step_2(true_idx))) + 1;
       info{nF}.rawData.nr_averages        = single(max(temp_raw_data.head.idx.average(true_idx))) + 1;
       info{nF}.rawData.nr_slices          = single(max(temp_raw_data.head.idx.slice(true_idx))) + 1; % imaging slice number
       info{nF}.rawData.nr_contrasts       = single(max(temp_raw_data.head.idx.contrast(true_idx))) + 1; % echo number in multi-echo
       info{nF}.rawData.nr_phases          = single(max(temp_raw_data.head.idx.phase(true_idx))) + 1; % cardiac phase
       info{nF}.rawData. nr_repetitions    = single(max(temp_raw_data.head.idx.repetition(true_idx))) + 1; % dynamic number for dynamic scanning
       info{nF}.rawData.nr_sets            = single(max(temp_raw_data.head.idx.set(true_idx))) + 1; % flow encoding set
       info{nF}.rawData.nr_segments        = single(max(temp_raw_data.head.idx.segment(true_idx))) + 1;
       info{nF}.rawData.trajectory_dimensions = single(max(temp_raw_data.head.trajectory_dimensions(true_idx))); % Get the dimensionality of the trajectory vector (0 means no trajectory)
       info{nF}.rawData.dt = single(max(temp_raw_data.head.sample_time_us(true_idx))) * 1e-3; %  dwell time in [sec][usec] * [msec/1e-3 usec] => [msec]
    end
    ...
    data = complex(zeros(info{nF}.rawData.nr_samples, info{nF}.rawData.nr_phase_encodings, info{nF}.rawData.nr_slice_encodings,...
        ...
        info{nF}.rawData.nr_repetitions, info{nF}.rawData.nr_sets, 1, info{nF}.rawData.nr_channels, 'double'));  %Remove segments temporily!!!
    
    if exist('true_idx')
        info{nF}.rawData.nr_profiles = length(temp_raw_data.data((true_idx)));
        for idx = 1:length(true_idx)
            idx
            % ---- Get information about the current profile ----
            phase_encoding_nr = single(temp_raw_data.head.idx.kspace_encode_step_1(true_idx(idx))) + 1;
            slice_encoding_nr = single(temp_raw_data.head.idx.kspace_encode_step_2(true_idx(idx))) + 1;
            average_nr        = single(temp_raw_data.head.idx.average(true_idx(idx))) + 1;
            slice_nr          = single(temp_raw_data.head.idx.slice(true_idx(idx))) + 1;
            contrast_nr       = single(temp_raw_data.head.idx.contrast(true_idx(idx))) + 1;
            phase_nr          = single(temp_raw_data.head.idx.phase(true_idx(idx))) + 1;
            repetition_nr     = single(temp_raw_data.head.idx.repetition(true_idx(idx))) + 1;
            set_nr            = single(temp_raw_data.head.idx.set(true_idx(idx))) + 1;
            segment_nr        = single(temp_raw_data.head.idx.segment(true_idx(idx))) + 1;
            
            % ---- Set the index range of a profile ----
            number_of_samples = single(temp_raw_data.head.number_of_samples(true_idx(idx)));
            discard_pre       = single(temp_raw_data.head.discard_pre(true_idx(idx)));
            discard_post      = single(temp_raw_data.head.discard_post(true_idx(idx)));
            start_index       = discard_pre + 1;
            end_index         = number_of_samples - discard_post;
            index_range       = (start_index:end_index).';
            
            % ---- Get k-space data ----
            full_profile = temp_raw_data.data{true_idx(idx)}; % number_of_samples x nr_channels
            data(index_range, phase_encoding_nr, slice_encoding_nr, average_nr, slice_nr,...
                contrast_nr, phase_nr, repetition_nr, set_nr, segment_nr==contrast_nr, :) = full_profile(index_range,:); % nr_samples x nr_channels
        end
    else
...
%     end
    ...
    if flags.Squeeze
        ...
        kdata{nF} = reshape(kdata{nF},...
            [info{nF}.rawData.nr_samples, info{nF}.rawData.nr_phase_encodings, info{nF}.rawData.nr_slices, info{nF}.rawData.nr_contrasts, info{nF}.rawData.nr_channels ]);
        % ---- Remove oversampling ----
        if flags.RemoveOS
            Nkx = size(kdata{nF},1);
            Nx = Nkx/2;
            imc_ = fftshift(fft(ifftshift(kdata{nF}, 1), [], 1), 1); % Nkx x Nky x Nslice x Ncontrast x Ncoil
            idx1_range = (-floor(Nx/2):ceil(Nx/2)-1).' + floor(Nkx/2) + 1;
            imc_ = imc_(idx1_range,:,:,:,:);  % Nx x Nky x Nslice x Nc
            kdata{nF} = fftshift(ifft(ifftshift(imc_, 1), [], 1), 1); % Nkx x Nky x Nslice x Ncontrast x Nc
        end
    end
